...
class BehaviorClassificationModel:
    ...
    def classify_behavior(self, frame, person_id, box):
        ...
        if self.classifier is None:
            self.classifier = pipeline(
                "image-classification", model=config.BEHAVIOR_MODEL, use_fast=True
            )
        ...
        if f"Person_{person_id}" in self.people_tracking:
            ...
            if (
                movement_x > config.MOVEMENT_THRESHOLD
                or movement_y > config.MOVEMENT_THRESHOLD
            ):
        ...
        if mask is not None and mask.size > 0 and len(mask.shape) == 2:
            try:
                ...
                print(f"OpenCV findContours error: {e}")
    ...
    def interpret_behavior(self, predictions):
        ...
        for pred in predictions:
            if any(
                indicator in pred["label"].lower() for indicator in panic_indicators
            ):
                return "Panicked"
        return "Normal"
